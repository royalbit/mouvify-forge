...
"#;
        ...
        // Tables should parse correctly
        assert_eq!(result.tables.len(), 1);
        assert!(result.tables.contains_key("data"));

        // Scalars should include both inputs and outputs
...
"#;
        ...
        // Check document names were used for prefixes
...
"#;
        ...
        // Auto-generated names should be doc1, doc2, etc.
...
"#;
        ...
        let result = parse_model(temp_file.path());
        // Schema validation catches missing 'as' field
        assert!(result.is_err());
    }
...
"#;
        ...
        let result = parse_model(temp_file.path());
        // Schema validation catches invalid format
        assert!(result.is_err());
    }
...
"#;
        ...
        let result = parse_model(temp_file.path());
        // Schema validation catches invalid value types in scenarios
        assert!(result.is_err());
    }
...
"#;
        ...
        let result = parse_model(temp_file.path());
        // Schema validation catches invalid scenario format
        assert!(result.is_err());
    }
...
"#;
        ...
        let result = parse_model(temp_file.path());
        // The schema validation might catch this first
        assert!(result.is_err());
    }
    ...
    fn test_detect_array_type_unsupported() {
        // Sequences are not valid array elements
    ...
    fn test_is_nested_scalar_section_false_for_v4_rich_column() {
        // v4.0 rich table column has value as array
        let mut map = serde_yaml::Mapping::new();
    ...
    fn test_is_nested_scalar_section_true_for_scalar() {
        // Scalar section has value as number
        let mut map = serde_yaml::Mapping::new();
        let mut child = serde_yaml::Mapping::new();
...
"#;
        ...
        let result = parse_model(temp_file.path()).unwrap();
        // Should have parsed the second document only
...
"#;
        ...
        let result = parse_model(temp_file.path()).unwrap();
        // Scenarios should be prefixed with document name
    ...
    fn test_parse_table_named_scenarios_as_table() {
        // A table named "scenarios" that has arrays (not scenario overrides)
        let yaml_str = r#"
_forge_version: "5.0.0"
scenarios:
  year: [2023, 2024, 2025]
...
"#;
        ...
        // Should be parsed as a table, not as scenarios
    ...
    fn test_is_nested_scalar_section_empty_child() {
        ...
        // Should not be considered a nested scalar section
        assert!(!is_nested_scalar_section(&map));
    }

    #[test]
    fn test_parse_table_column_scalar_not_array() {
        // Table column as scalar value should error
        let mut map = serde_yaml::Mapping::new();
